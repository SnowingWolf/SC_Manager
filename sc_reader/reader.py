...
class SCReader(MySQLReader):
    """
    慢控数据读取器，扩展自 MySQLReader
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            **kwargs: 传递给 MySQLReader 的其他参数（可覆盖 config）
                - host: 数据库主机
                - user: 用户名
                - password: 密码
                - database: 数据库名
    ...
    def get_table_info(self, table_name: str) -> pd.DataFrame:
        ...
        Examples:
            ...
            >>> print(info)
        """
        structure = self.describe_table(table_name)
        return pd.DataFrame(
            structure,
    ...
    def preview_data(self, table_name: str, limit: int = 5) -> pd.DataFrame:
        ...
        Examples:
            >>> data = reader.preview_data('temperature_table', limit=10)
            >>> print(data.head())
        """
        # 获取时间列名
    ...
    def _query_single_table(
        ...
        if start_time:
            ...
            if is_string_type:
                # 如果时间列是字符串类型，使用 STR_TO_DATE 进行转换和比较
                where_conditions.append(
                    f"STR_TO_DATE(`{time_col}`, '%Y/%m/%d %H:%i:%s') >= STR_TO_DATE('{start_time_str}', '%Y-%m-%d %H:%i:%s')"
                )
            else:
        ...
        if end_time:
            ...
            if is_string_type:
                # 如果时间列是字符串类型，使用 STR_TO_DATE 进行转换和比较
                where_conditions.append(
                    f"STR_TO_DATE(`{time_col}`, '%Y/%m/%d %H:%i:%s') <= STR_TO_DATE('{end_time_str}', '%Y-%m-%d %H:%i:%s')"
                )
            else:
                # 如果时间列是 datetime 类型，直接比较
    ...
    def get_columns(self, table_name: str) -> List[str]:
        ...
        Examples:
            >>> columns = reader.get_columns('temperature_table')
            >>> print(f"可用的列: {', '.join(columns)}")
        """
        info = self.get_table_info(table_name)
        return info['Field'].tolist()
    ...
    def read_incremental(
        ...
        if wm['last_ts'] is not None:
            ...
            if spec.time_unit:
                ...
                # 使用 STR_TO_DATE 兼容字符串时间列
                where_parts.append(
                    f"STR_TO_DATE(`{time_col}`, '%Y/%m/%d %H:%i:%s') >= '{cutoff_str}'"
                )
    ...
    def read_multiple(
        ...
        """
        return {spec.table: self.read_incremental(spec, lookback) for spec in specs}
    ...
    def save_state(self, path: Optional[str] = None):
        ...
        for table, wm in self._watermarks.items():
            state[table] = {
                'last_ts': wm['last_ts'].isoformat() if wm['last_ts'] else None,
                'last_id': wm['last_id']
            }
